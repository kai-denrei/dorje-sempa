...
# ---------- 2b. Bump the service-worker cache-bust token (keys the SW cache name) ----------
# sw.js carries `const CB_TOKEN = '<token>'`; the cache name is `the-path-<token>`,
# so rewriting it here makes every deploy invalidate the old SW cache cleanly.
# fingerprint-urls.py only touches .html/.css, so the SW token needs this hook.
if [[ -f sw.js ]]; then
